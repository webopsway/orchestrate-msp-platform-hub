...
export function IncidentAssignment({ 
  ...
  const assignIncident = async (assigneeId: string | null) => {
    try {
      setAssigning(true);
      ...
      toast.success(assigneeId ? 'Incident assigné avec succès' : 'Assignation supprimée');
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-64 p-0" side="bottom" align="start">
        <div className="p-3 border-b">
          <h4 className="font-medium text-sm">Assigner l'incident</h4>
            ...
            <>
              ...
              {users.map((user) => (
                <Button
                  ...
                  onClick={() => assignIncident(user.id)}
                  disabled={assigning}
                >
